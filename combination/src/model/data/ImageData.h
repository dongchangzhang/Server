...
public:
    ...
    inline bool get_pixel(int ncol, int nrow, unsigned char &r, unsigned char &g, unsigned char &b) {
        if (!data || ncol >= width || nrow >= height) {
            return false;
        } else {
            r = data[(nrow * width + ncol) * 3 + 0];
            g = data[(nrow * width + ncol) * 3 + 1];
            b = data[(nrow * width + ncol) * 3 + 2];
            return true;
        }
